...
public class RRRPMainClass extends JavaPlugin implements Listener {
	...
	private void giveItemPlayer(Player p, ItemStack i) {
	...
	private void rankperkAll(String string, String[] args, CommandSender sender) {
		...
		for (Player p : Bukkit.getOnlinePlayers()) {
			...
			case "wood":
				giveItemPlayer(p, perksClass.woodPerkItem(p));
				break;

			case "stone":
				giveItemPlayer(p, perksClass.stonePerkItem(p.getDisplayName()));
				break;

			case "bronze":
				giveItemPlayer(p, perksClass.bronzePerkItem(p.getDisplayName()));
				break;
			}
		}
		if (args.length == 1) {
	...
	public void onPlayerThrowEgg(PlayerEggThrowEvent event) {
		...
		if (thrower.getItemInHand() != null
				|| thrower.getItemInHand().getType() != Material.AIR) {
			if (thrower
					.getItemInHand()
					.getItemMeta()
					.getDisplayName()
