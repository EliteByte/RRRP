...
public class TabCompletion implements TabCompleter {
							...
							if (plugin.getConfig().get(args[2]) != null) {
								if (args[2].equals("") || args[2].endsWith(".")) {
									sender.sendMessage(ChatColor.GRAY + ""
											+ getAsFullPathList(args[2], false)
											+ ChatColor.DARK_GRAY + " : "
											+ "Current Config Paths");
									return getAsFullPathList(args[2], false);
								} else {
									if (getAsFullPathList(args[2], false)
											.isEmpty()) {
										sender.sendMessage(ChatColor.DARK_RED
												+ "End Of Path, Current Path's Value = "
												+ ChatColor.DARK_GRAY
	...
	public List<String> getAsFullPathList(final String path, boolean bool) {
		...
		if (plugin.getConfig().contains(path)
			...
			final Set<String> keys = plugin.getConfig()
